...
# 3.读取每个软件;
# 3.1 BroCOLI_v2(改动了之后)
# BroCOLI_transcript_counts_path <- "D:/AAAAABroCOLICode/SIRV_Sampled_Annotation_Fig2/Quantification/BroCOLI_v2/counts_transcript.txt"
...
# BroCOLI_transcript_counts_path <- "D:/AAAAABroCOLICode/SIRV_Sampled_Annotation_Fig2/Quantification/BroCOLI/counts_transcript_n2.txt"
# BroCOLI_transcript_counts_path <- "D:/AAAAABroCOLICode/SIRV_Sampled_Annotation_Fig2/Quantification/BroCOLI/counts_transcript_n1.txt"
# BroCOLI_transcript_counts_path <- "D:/AAAAABroCOLICode/SIRV_Sampled_Annotation_Fig2/Quantification/BroCOLI_v2/counts_transcript_n6.txt"
# BroCOLI_transcript_counts_path <- "D:/AAAAABroCOLICode/SIRV_Sampled_Annotation_Fig2/20250805/counts_transcript.txt"
# BroCOLI_transcript_counts_path <- "D:/AAAAAAAAAAAAAAAAAAAAAAA_LW/counts_transcript.txt"
BroCOLI_transcript_counts_path <- "D:/AAAAABroCOLICode/SIRV_Sampled_Annotation_Fig2/Quantification/BroCOLI/counts_transcript.txt"
...
cor(data_plot$Truth, data_plot$Predict, method = "pearson")
cor(data_plot$Truth, data_plot$Predict, method = "spearman")
...
ggplot(data_plot,aes(x=Truth_xlabel, y=Predict)) + 
  ...
  annotate("text", label= expression(paste("Spearman'", rho, " = 0.898")), x="1/4", y=5000, size=6)
dev.off()
# 以上是定量图;
...
data_plot <- merge(Answer, Bambu_predict, by=c("Transcript"))
cor(data_plot$Truth, data_plot$Predict, method = "pearson")
cor(data_plot$Truth, data_plot$Predict, method = "spearman")
data_plot$Truth <- as.factor(data_plot$Truth)

cor(as.numeric(data_plot$Truth), as.numeric(data_plot$Predict), method = "pearson")
cor(as.numeric(data_plot$Truth), as.numeric(data_plot$Predict), method = "spearman")
...
ggplot(data_plot,aes(x=Truth_xlabel, y=Predict)) + 
  ...
  scale_size_manual(values = c(1, 2, 3, 4, 5), 
                    ...
                    labels = c("500", "1000", "1500", "2000", "2500") ) +
  guides(fill=FALSE, color=FALSE) +
  annotate("text", label= expression(paste("Spearman'", rho, "=0.778")), x="1/4", y=5000, size=6)
dev.off()
# 以上是定量图;
# 下面是计算Annotation百分百时候的sensitivity和precision
# 预测的\真实的   Positive   Negative   真实的
...
data_plot <- merge(Answer, Isoquant_predict, by=c("Transcript"))
cor(data_plot$Truth, data_plot$Predict, method = "pearson")
cor(data_plot$Truth, data_plot$Predict, method = "spearman")
data_plot$Truth <- as.factor(data_plot$Truth)
...
ggplot(data_plot,aes(x=Truth_xlabel, y=Predict)) + 
  ...
  scale_size_manual(values = c(1, 2, 3, 4, 5), 
                    ...
                    labels = c("500", "1000", "1500", "2000", "2500") ) +
  guides(fill=FALSE, color=FALSE) +
  annotate("text", label= expression(paste("Spearman'", rho, "=0.895")), x="1/4", y=5000, size=6)
dev.off()
# 以上是定量图;
# 下面是计算Annotation百分百时候的sensitivity和precision
# 预测的\真实的   Positive   Negative   真实的
...
Isoquant_predict <- Isoquant_predict[-(67:69), ]
...
data_plot <- merge(Answer, Isosceles_predict, by=c("Transcript"))
cor(data_plot$Truth, data_plot$Predict, method = "pearson")
cor(data_plot$Truth, data_plot$Predict, method = "spearman")
data_plot$Truth <- as.factor(data_plot$Truth)
...
ggplot(data_plot,aes(x=Truth_xlabel, y=Predict)) + 
  ...
  scale_size_manual(values = c(1, 2, 3, 4, 5), 
                    ...
                    labels = c("500", "1000", "1500", "2000", "2500") ) +
  guides(fill=FALSE, color=FALSE) +
  annotate("text", label= expression(paste("Spearman'", rho, "=0.788")), x="1/4", y=5000, size=6)
dev.off()
# 首先将预测的大于0的保留下来;
# 这个也需要做特殊处理;
...
data_plot <- merge(Answer, StringTie_predict, by=c("Transcript"))
cor(data_plot$Truth, data_plot$Predict, method = "pearson")
cor(data_plot$Truth, data_plot$Predict, method = "spearman")
data_plot$Truth <- as.factor(data_plot$Truth)
...
ggplot(data_plot,aes(x=Truth_xlabel, y=Predict)) + 
  ...
  scale_size_manual(values = c(1, 2, 3, 4, 5), 
                    ...
                    labels = c("500", "1000", "1500", "2000", "2500") ) +
  guides(fill=FALSE, color=FALSE) +
  annotate("text", label= expression(paste("Spearman'", rho, "=0.706")), x="1/4", y=5000, size=6)
dev.off()
# 首先将预测的大于0的保留下来;
